...
app.post("/api/deleteAlumniMember", (req, res) => {
  ...
  con.query(`SELECT profileImage from alumni WHERE cmsId= ?`, [cms], (err, row, col) => {
    if (err) res.send("Network error occured!");
    if (row) {
     ...
     if(imgAddress){
      ...
      con.query(`DELETE from alumni WHERE cmsId= ?`, [cms], (err, row, col) => {
        if (err) res.send("Network error occured!");
        if (row) {
...
//Search Alumni
app.get("/api/searchAlumni", (req, res) => {});
